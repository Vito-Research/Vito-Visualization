...
def analyze():
   ...
   # RiskFile = st.file_uploader("Upload Risk Data", type=("csv"))
    ...
    # a dictionary
       ...
       # df_merged = df_merged.drop('Start_Time_Risk', 1)
        ...
        if df_merged.shape[0] > 5:
            averages["averages"].append(accuracy_score(df_merged["Risk"], df_merged["NS Alerts"]))
    ...
    st.sidebar.header("Accuracy: " + str(accuracy_score(allDF["Risk"], allDF["NS Alerts"])))
    st.sidebar.header("Kappa: " + str(cohen_kappa_score(allDF["Risk"], allDF["NS Alerts"])))
    st.sidebar.header("Precision: " + str(precision_score(allDF["Risk"], allDF["NS Alerts"])))
    st.sidebar.header("Recall: " + str(recall_score(allDF["Risk"], allDF["NS Alerts"])))
